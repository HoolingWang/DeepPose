...
class AffineTransform(object):
    """仿射变换shift+scale+rotation"""
    def __init__(self,
                 scale_factor: Tuple[float, float] = (0.65, 1.35),   # 缩放
                 scale_prob: float = 1.,   # 缩放概率
                 rotate: int = 45,    # 旋转
                 rotate_prob: float = 0.6,
                 shift_factor: float = 0.15,   # 平移
                 shift_prob: float = 0.3,
                 fixed_size: Tuple[int, int] = (256, 256)   # 输出尺寸
    ):   
